...
import mlflow
import torch
from cv_pipeline.train import SegmentationDataset, _compute_metrics, train
...
def evaluate_on_test(
    ...
    with torch.no_grad():
        for images, masks in test_loader:
            ...
            logits = model(images)
            f1, iou = _compute_metrics(logits, masks)

            f1_sum += f1
            iou_sum += iou
            batches += 1

    test_f1 = f1_sum / max(batches, 1)
    test_iou = iou_sum / max(batches, 1)
